...
当前已经具备多领域调度器、统一流式 Agent 适配器、受限 Semantic Judge、三轮结果聚合与
脱敏正式证据写入能力；正式 Runner 可按 Case 创建独立 SQLite，隔离订单、记忆与人工接管
写入，并通过显式工厂固定知识与预约依赖。目前尚未提交 200 条合规 Golden Set，也未准备
与该 Golden Set 对应的 200 份真实工具快照和 Baseline 实跑结果；工具快照的严格契约、
版本绑定、HermesRAG 离线返回及预约人员/排班隔离装载能力已经完成。
...
语义评测器已经提供受限适配器，只用于“答案相关性”和“必要信息完整性”这两类无法由
工具轨迹精确判断的指标。它要求模型仅返回严格 JSON 分数和枚举原因码，不保存分析过程；
超时、异常、非法 JSON、额外字段或越界分数统一记为 `unknown`，不能计为通过。仓库内
6 条确定性 Smoke Case 不调用语义模型，因此日常回归仍可完全离线运行。
